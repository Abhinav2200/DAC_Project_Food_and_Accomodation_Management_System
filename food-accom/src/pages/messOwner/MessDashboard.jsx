...
export default function MessDashboard() {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            console.log("Fetching dashboard data for User ID:", user.userId);

            const results = await Promise.allSettled([
                axios.get(`http://localhost:8080/api/mess/menu/owner/${user.userId}`),
                axios.get(`http://localhost:8080/api/bookings/owner/${user.userId}`),
                axios.get(`http://localhost:8080/api/ratings/${user.userId}`)
            ]);

            const [menuRes, bookingsRes, ratingsRes] = results;

            // Handle Menu Stats
            if (menuRes.status === 'fulfilled') {
                console.log("Menu Data:", menuRes.value.data);
                ...
                console.error("Menu API Failed:", menuRes.reason);
            }
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
            ...
            <div style={{ display: "flex", gap: "20px", marginBottom: "30px", flexWrap: "wrap" }}>
                <div style={statBoxStyle}>
                    <span style={statLabelStyle}>Total Dishes</span>
                    <span style={statValueStyle}>{stats.total}</span>
                </div>
                <div style={{ ...statBoxStyle, borderLeftColor: "#e056fd" }}>
                    <span style={statLabelStyle}>Available Today</span>
            ...
            <div style={{
                ...
                <DashboardCard
                    title="Manage Menu"
                    desc="Add dishes, update prices and weekly menu"
                    onClick={() => navigate("/owner/mess/menu")}
                    color="#e056fd"
                    icon="🍽️"
                />
            </div>
...
const DashboardCard = ({ title, desc, onClick, color, icon }) => (
    <div
        ...
        style={{
            ...
            cursor: "pointer",
            borderTop: `5px solid ${color}`, // Thinner border
            transition: "all 0.2s",
            display: "flex",
